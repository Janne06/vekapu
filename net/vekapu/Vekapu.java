...
public class Vekapu {
		...
		if (args.length > 0)
	      {
	         ...
	         for (int i = 0; i < args.length; i++)
	         {
	        	 ...
	        	 if ( args[i].equalsIgnoreCase("-version")  ) version = true;
	        	 if ( args[i].equalsIgnoreCase("-v")  ) version = true;
	...
	public static void main(String[] args) throws VekapuException {
	...
	public void start() throws VekapuException {
		...
		if (settingsVO.getGroupInfo().equals("")) {
			...
			OwnNumbers ownNbr = new OwnNumbers(settingsVO.getGroupInfo(),settingsVO);
	...
	public void tarkista(String kierros) throws VekapuException {
		...
		for (int i = 1; i <= settingsVO.getGroupCount().intValue(); i++) {
			...
			if (dayhelper.isSaturday() && 
					settingsVO.isCronJob().booleanValue()) {
				logger.info("Lauantai cronilla == true ==> tarkistetaan lauantain pelit (Lotto & Jokeri)");
				tarkista = true; 
			}
			...
			if (tarkista) {
				...
				if (numbersVO.isLotto()) {
					...
					if (dayhelper.isExpired(numbersVO.getMihinAstiLotto())) {
						Messenger.sendEndMail("Lotto", group, numbersVO.getTo(), settingsVO);
				...
				if (numbersVO.isJokeri()) {
					...
					parasJokeri = " Jokeri '" + String.valueOf(resultVO.getJokeribest()) + "' kpl";
			...
			if (dayhelper.isSaturday() && 
					settingsVO.isCronJob().booleanValue()) {
				logger.info("Keskiviikko cronilla == true ==> tarkistetaan keskiviikon peli (VikingLotto)");
				tarkista = true; 
			}
			...
			if (checked) {
				...
				if (settingsVO.isServer().booleanValue()) {
					dir = settingsVO.getGroupDir() + "/" + group;
				} else {
					dir = ".";
				}
				...
				// Nyt tutkitaan vain Loton loppumisajankohta. Tosin VO:ssa katotaan
				...
				if (numbersVO.isStoreResult()) {
					logger.info("Talleteaan tulokset kierrokselta: " + settingsVO.getWeek());
				...
				if (settingsVO.isEmail().booleanValue() || settingsVO.isTest().booleanValue()) {
					int len = kierros.length();
					logger.debug("Kierros: " + kierros + " : " + len);
					if (len > 10) 
					   len = 10;
					otsikko = kierros.substring(0, len).trim() + " Parastulos "
							+ parasTulos;

					if (numbersVO.isJokeri())
						otsikko = otsikko + parasJokeri;
					logger.debug("Otsikko: " + otsikko);

					Messenger.sendEMail(otsikko, group, numbersVO.getTo(), resultVO.toString(), settingsVO);
				}

				// Lähetetäänkö tekstivietejä ??????????????????
				if (settingsVO.isSms().booleanValue() || settingsVO.isTest().booleanValue()) {
					otsikko = kierros.substring(0, 10).trim() + " Parastulos "
							+ parasTulos;
					...
					Messenger.sendSMS(otsikko, group, numbersVO.getToSms(), settingsVO);
				}
			}
			
			// Katotaan kuinka tarkistus menis tällätavalla
			logger.debug(resultVO.getHeader() + resultVO.printLotto() + 
					resultVO.printJokeri(false) + resultVO.printViking());
